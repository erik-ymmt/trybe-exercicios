...
const { registerCar, findCar } = require('./models/carModel');
const connection = require('./models/connection');
const { findAllDrivers, findDriver, registerNewDriver } = require('./models/driversModel');
...
app.post('/drivers', async (req, res) => {
  ...
  const newDriverId = await registerNewDriver(newDriver);
  res.status(201).json(newDriverId);
});

app.post('/cars', async (req, res) => {
  const newCar = req.body;
  const newCarId = await registerCar(newCar);
